    ...
    <>
      <section className="bg-light overflow-hidden py-14 lg:py-18 xl:22 bg-prim-light">
        <div className="container mx-auto lg:max-w-(--breakpoint-xl) md:max-w-(--breakpoint-md) px-4 space-y-14">
          ...
          <div className="blog-wrapper grid md:grid-cols-2 grid-cols-1 gap-2">
            {blogs.slice(0, 1).map((item) => (
              <div
                key={item.id}
                className="blog-item w-full h-auto md:h-[640px] shadow-lg bg-white p-5 rounded-2xl group "
              >
                <div className="blog-image w-full rounded-2xl overflow-hidden relative">
                  ...
                  <span className="absolute right-5 bottom-5 max-w-sm text-white font-chakrapetch font-semibold bg-white/10 backdrop-blur-sm rounded-sm text-2xl text-center p-3">
                    {item.date}
                  </span>
                </div>
                ...
                <div className="blog-content px-2 py-5 space-y-5">
                  <p>
                    <span className="border border-border px-2 mr-2 rounded-sm">
                      {item.category}
                    </span>
                    By {item.author}
                  </p>
            ...
            <div className="grid grid-cols-1 h-auto md:h-[620px] gap-5">
              {blogs.slice(1).map((item) => (
                ...
                >
                  ...
                  <div className="blog-content px-2 py-5 space-y-5">
                    <p>
                      <span className="border border-border px-2 rounded-sm mr-2">
                        {item.category}
                      </span>
                      By {item.author}
                    </p>

                    <h4 className="font-unbounded font-medium pb-2 text-20">
                      {item.title}
                    </h4>
